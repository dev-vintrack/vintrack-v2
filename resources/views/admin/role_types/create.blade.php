...
@section('title', 'Nuevo Tipo de Rol')

@section('content')
<div class="row justify-content-center">
    <div class="col-md-8">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h2 class="mb-0">Nuevo Tipo de Rol</h2>
            <a href="{{ route('admin.role-types.index') }}" class="btn btn-outline-secondary btn-sm">Regresar</a>
        </div>

        <div class="card border-0 shadow-sm">
            <div class="card-body">
                <form action="{{ route('admin.role-types.store') }}" method="POST">
                    @csrf

                    <div class="mb-3">
                        <label for="type_name" class="form-label fw-semibold">Nombre del tipo <span class="text-danger">*</span></label>
                        <input type="text" name="type_name" id="type_name" class="form-control" maxlength="32" value="{{ old('type_name') }}" required>
                    </div>

                    <div class="mb-3">
                        <label for="description" class="form-label fw-semibold">Descripción</label>
                        <input type="text" name="description" id="description" class="form-control" maxlength="255" value="{{ old('description') }}">
                    </div>

                    <div class="mb-3">
                        <label for="color" class="form-label fw-semibold">Color (clase Bootstrap)</label>
                        <input type="text" name="color" id="color" class="form-control" maxlength="32" value="{{ old('color') }}" placeholder="Ej: danger, info, primary, warning">
                    ...
                    <div class="row mb-3">
                        <div class="col-md-6">
                            <div class="form-check">
                                <input class="form-check-input" type="checkbox" name="is_admin" id="is_admin" value="1" {{ old('is_admin', '1') == '1' ? 'checked' : '' }}>
                                <label class="form-check-label fw-semibold" for="is_admin">Es tipo administrativo</label>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="form-check">
                                <input class="form-check-input" type="checkbox" name="is_customer" id="is_customer" value="1" {{ old('is_customer') == '1' ? 'checked' : '' }}>
                    ...
                    <div class="d-flex justify-content-end">
                        <button type="submit" class="btn btn-primary">Guardar Tipo de Rol</button>
                    </div>
                </form>
            </div>
        </div>
