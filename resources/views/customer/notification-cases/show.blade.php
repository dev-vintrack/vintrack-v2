...
<ul class="list-group" id="evidence-list">@forelse($documents as $document)<li class="list-group-item d-flex justify-content-between align-items-center"><span>{{ $document['original_name'] }} ({{ number_format($document['size_bytes']/1024, 1) }} KB) <span class="badge text-bg-{{ $document['malware_scan_status'] === 'CLEAN' ? 'success' : ($document['malware_scan_status'] === 'INFECTED' ? 'danger' : 'warning') }}">{{ $document['malware_scan_status'] }}</span></span><span>@if($document['download_available'])<a class="btn btn-sm btn-outline-primary" href="{{ route('notification-cases.documents.show', [$case, $document['id']]) }}">Descargar</a>@endif @if($editable)<button type="button" class="btn btn-sm btn-outline-danger remove-document" data-id="{{ $document['id'] }}">Remover</button>@endif</span></li>@empty<li class="list-group-item text-muted">Sin evidencias.</li>@endforelse</ul>
</div></div>
